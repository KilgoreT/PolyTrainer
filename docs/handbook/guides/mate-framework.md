...
Подробнее — `docs/handbook/guides/effect-handlers.md`, `docs/handbook/specs/navigation.md`.
...
6. **FlowHandler'ы** автоматически подписываются при инициализации Mate и отписываются при dispose.
   ...
   **⚠ Автоподписка работает ТОЛЬКО для handler'ов из `effectHandlerSet`.** Если `MateFlowHandler` создан как класс и заинжектирован в ctor другого handler'а (например в `DatasourceEffectHandler`), но НЕ добавлен в `effectHandlerSet` ViewModel — `subscribe()` никогда не вызовется. Flow не стартует, Msg не приходят, state-поле остаётся в initial value. **Все unit-тесты на Reducer останутся зелёными**, потому что они не покрывают wiring. Это поймает только runtime / integration test. См. `R-MF-001`.
...
### R-MF-001 — FlowHandler обязан быть в effectHandlerSet ViewModel

- **Severity:** critical
- **Applies to:** любой класс реализующий `MateFlowHandler<Msg, Effect>` (прямо или через `MateTypedEffectHandler`).
- **Check:** для каждого `class XxxFlowHandler @Inject constructor(...) : MateFlowHandler<...>` через `Grep "XxxFlowHandler" --glob "**/*ViewModel.kt"` найти `effectHandlerSet = setOf(...)` хотя бы в одной ViewModel и убедиться что `XxxFlowHandler` там присутствует. Если handler только инжектится в ctor другого handler'а / use case / DI module — но в `effectHandlerSet` его нет — **нарушение**.
- **Почему:** `Mate.subscribeToLongRunningFlows()` подписывает на flow только handler'ы из `effectHandlerSet`. Без регистрации `subscribe()` не вызовется → flow не стартует → state-поле навсегда initial → unit-тесты на Reducer всё равно зелёные (они только реагируют на Msg, не проверяют доехал ли он). Поймает только runtime / integration test.
- **Fix:** добавить handler в `effectHandlerSet` ViewModel. Если планировалось дёргать его через специальный `Effect.SubscribeXxx` — убедиться что Effect эмитится Reducer'ом / `initEffects`; иначе Effect dead code.
