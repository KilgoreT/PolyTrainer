...
8. **Boolean колонки** объявляются с `CHECK (<col> IN (0, 1))` для defense-in-depth. Room хранит `Boolean` как `INTEGER` (0/1); CHECK защищает от corrupt data при прямом доступе (ADB sqlite3, raw migration script). Применяется ко всем новым таблицам и при добавлении новой boolean-колонки в существующую таблицу. Пример: `is_multi INTEGER NOT NULL DEFAULT 0 CHECK (is_multi IN (0, 1))`.
9. **Timestamps** в таблицах с пользовательскими данными — convention `created_at` / `updated_at` / `removed_at` (`INTEGER`, Unix ms). `removed_at NULL` = активная запись; не NULL = soft-delete. `updated_at` обновляется в repository/DAO, не БД-trigger.
10. **Template immutable после релиза.** После того как `ComponentTemplate` entry (например `QUOTE_WITH_SOURCE`) попал в релиз — его `fields` (имена, типы, порядок, состав) **изменять запрещено**. Нужны другие поля / иная структура → создаётся **новый template** (новое entry в enum, новый widget, новый `*Values` data class). Старый template продолжает работать как был. Причина: existing JSON в `component_values.value` зашит на текущую schema; любое изменение schema (rename / add field / remove field / type change) = schema-mismatch и silent breakage на все existing rows. Защита — golden fixture round-trip test в `core-db-impl` (`fixtures/component_values/*.json` хранит ожидаемые JSON per known template, парсер сравнивает с typed view, любое изменение existing template = CI failure).
