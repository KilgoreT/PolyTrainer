...
- **[Модуль guides почти не обновляет гайды].**
  Модуль guides подключен к большинству шагов flow, но на практике conductor/агенты крайне редко добавляют или обновляют гайды в `docs/guides/`. Артефакты фичи создаются, а переиспользуемые паттерны не фиксируются.
  Нужно: усилить промпт модуля guides — явно требовать от агента проверку "есть ли в этом шаге паттерн, достойный гайда?" и выдавать конкретный diff в гайд, а не просто ставить галочку.
