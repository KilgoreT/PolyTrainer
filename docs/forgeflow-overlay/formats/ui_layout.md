...
- **Никаких тегов `⟦...⟧`** после имени виджета. Старые теги (`⟦chip+input⟧`, `⟦container+text⟧`) **запрещены** — они приводят к разной интерпретации семантики. Структура каждого виджета описывается явно в § 4 «Анализ виджетов» через поле `structure:` с примитивами из гайда (см. `docs/guides/ui-primitives.md`).
...
```
   • structure:   ↓ обязательное поле для каждого custom widget. Описание через примитивы
                  и слоты из гайда (см. `docs/guides/ui-primitives.md`).
...
6. **Структура:** для каждого custom widget (маркеры ❇️ / 🔄 / 📌) в `## 🔍 Анализ виджетов` присутствует поле `structure:` с описанием через примитивы и слоты (см. `docs/guides/ui-primitives.md`). Generic `container`, старые теги `⟦...⟧` и atom-only widget — запрещены.
