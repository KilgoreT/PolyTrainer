---
name: ui_layout
description: "Канон формы UI Layout документа. Описывает структуру, маркеры, шаблон буллетов и договорённости."
---
...
- **Никаких тегов `⟦...⟧`** после имени виджета. Старые теги (`⟦chip+input⟧`, `⟦container+text⟧`) **запрещены** — они приводят к разной интерпретации семантики. Структура каждого виджета описывается явно в § 4 «Анализ виджетов» через поле `structure:` с примитивами из гайда (см. `docs/handbook/guides/ui-primitives.md`).
...
```
⚙️ Scaffold
├─ 🔄 <𝗧𝗼𝗽𝗕𝗮𝗿>                                title="Items", actions=[⋮]
└─ ⚙️ Column                                  padding=h:16  spacing=12
   ├─ 📌 <𝗦𝗲𝗮𝗿𝗰𝗵𝗙𝗶𝗲𝗹𝗱>                       hint="Search…"
   │
   └─ ⚙️ LazyColumn (× N)                     spacing=8
      🔄 <𝗜𝘁𝗲𝗺𝗖𝗮𝗿𝗱>                          padding=16  spacing=8
      ├─ ❇️ <𝗧𝗶𝘁𝗹𝗲𝗖𝗵𝗶𝗽>                      text="Active"
      └─ 📌 <𝗗𝗲𝗹𝗲𝘁𝗲𝗕𝘂𝘁𝘁𝗼𝗻>
   ↘️ FAB-slot                                pos=BottomEnd
   └─ ❇️ <𝗔𝗱𝗱𝗜𝘁𝗲𝗺>
```

Что показано: ⚙️ для системных узлов, ❇️/🔄/📌 для кастомных, Unicode bold без тегов, slot-style params после имени, `(× N)` для динамики, размеры родительского контейнера справа от узла. **Структура каждого custom-виджета** (из чего он состоит) — в § 4 «Анализ виджетов» через поле `structure:`.
...
Маркер + имя в Unicode bold (без тегов):
...
```
   • structure:   ↓ обязательное поле для каждого custom widget. Описание через примитивы
                  и слоты из гайда (см. `docs/handbook/guides/ui-primitives.md`).
                  Формат: корневой layout-примитив + именованные слоты с вложенными примитивами.
                  Пример:
                       column  spacing=4
                         header_slot: chip  variant=SubentityChip  label="Translation"
                         value_slot: text-or-input (mode-dependent)
                         action_slot: icon  position=trailing  iconRes=ic_close
                  Запрещены: generic `container`, теги `⟦chip+input⟧`, atom-only widget.
...
Поле `slots/content:` из прошлой версии заменено на `structure:` — оно содержит полную иерархию слотов в одном поле, нет смысла дублировать.
...
🔄 <𝗜𝘁𝗲𝗺𝗖𝗮𝗿𝗱>
   • structure:
       row  spacing=8  padding=16
         leading_slot:  chip  variant=TitleChip
         content_slot:  text  source=item.title
         trailing_slot: button  variant=DeleteButton
   • type:        M3 Surface (Card) с row внутри
...
❇️ <𝗧𝗶𝘁𝗹𝗲𝗖𝗵𝗶𝗽>
   • structure:
       box
         content_slot: chip  variant=M3-InputChip  label=text
   • type:        M3 InputChip
...
Виджет с последовательностью детей в column:

```
🔄 <𝗦𝘁𝗮𝗰𝗸𝗖𝗮𝗿𝗱>
   • structure:
       column  spacing=12  padding=16
         slot_0: chip   variant=TitleChip
         slot_1: text   source=item.body
         slot_2: button variant=DeleteButton
...
5. **Карта экрана:** все кастомные виджеты в Unicode bold (без тегов `⟦...⟧`), системные — обычным шрифтом.
6. **Структура:** для каждого custom widget (маркеры ❇️ / 🔄 / 📌) в `## 🔍 Анализ виджетов` присутствует поле `structure:` с описанием через примитивы и слоты (см. `docs/handbook/guides/ui-primitives.md`). Generic `container`, старые теги `⟦...⟧` и atom-only widget — запрещены.
