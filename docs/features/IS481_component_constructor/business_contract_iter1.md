...
sealed interface Msg {
    ...
    data class TypesLoaded(val types: List<ComponentType>, val usage: ComponentUsage) : Msg
...
Аналогичный sealed `Msg` с теми же `Open*Dialog` / `Submit*` / `*Result` событиями. Отличие — init: `data class ItemsLoaded(val items: List<PerDictRow>, val dictionaryName: String) : Msg`. Подробно — на business_implement, набор reducer-веток зеркальный.
...
| `CreateResult(Success(type))` | `isCreating=false, createDialog=null` | `UiEffect.Snackbar("Created")` |
| `CreateResult(NameTaken(scope))` | `isCreating=false, createDialog.copy(nameError=ScopeCollision)` | `∅` |
...
| `TypesLoaded(types, usage)` | `userDefinedTypes = types.toRows(usage), isLoading=false` | `∅` |
...
- `AllUserDefinedTypesFlowHandler` — подписывается на `useCase.flowAllUserDefinedTypes()` (вместе с aggregated usage) → emit `Msg.TypesLoaded` / `Msg.TypesLoadFailed`. Триггерится `ComponentsManagerDatasourceEffect.SubscribeAll` либо инициируется в `init` Mate.
...
sealed interface CreateOutcome {
    data class Success(val type: ComponentType) : CreateOutcome
...
import me.apomazkin.lexeme.ComponentType
import me.apomazkin.lexeme.ComponentTypeId
import me.apomazkin.components_manager.logic.ComponentUsage
import me.apomazkin.components_manager.logic.CreateOutcome
import me.apomazkin.components_manager.logic.DeleteOutcome
import me.apomazkin.components_manager.logic.DeletionImpact
import me.apomazkin.components_manager.logic.RenameOutcome
import me.apomazkin.components_manager.logic.Scope
...
interface ComponentsManagerUseCase {
     ...
     */
    fun flowAllUserDefinedTypes(): Flow<Pair<List<ComponentType>, ComponentUsage>>
     ...
     *
     * @return [CreateOutcome.Success] с domain entity OR typed error.
     */
...
interface PerDictionaryComponentsUseCase {
     ...
     *
     * @return pair: (types, valueCountByType, dictionaryName)
     */
...
sealed interface CreateComponentOutcome {
    data class Success(val type: ComponentTypeApiEntity) : CreateComponentOutcome
...
3. **`flow*` методы возвращают `Pair<...>` / dedicated data class?** Контракт использует dedicated data class (`UserDefinedTypesSnapshot`, `PerDictionarySnapshot`, `ComponentUsage`) — устраняет неоднозначность `.first/.second` в reducer. Подтверждается на business_design_tree.
...
5. **`AffectedQuizConfig.quizMode` enum source** — domain `QuizMode` уже существует в `modules/domain/quiz/` (предположительно); если нет — wiring на business_implement (просто Long либо String).
...
DONE — business_contract.md created.
