<!-- META: spec_filename: lexeme-domain.md -->
...
- **`LexemeMapper.toDomain()` unit-тесты** (в `app/src/test/java/me/apomazkin/polytrainer/mapper/LexemeMapperTest.kt`):
  - Маппит API → Domain для non-null `translation` и `definition` (значения сохраняются в `Translation.value` / `Definition.value`).
  - Маппит при `translation == null` и `definition == null` (получаем `null` в соответствующих полях domain).
  - Корректно оборачивает `id: Long` в `LexemeId`.
  - Сохраняет `wordId`, `addDate`, `changeDate` (включая `changeDate == null`).
- (Двойной маппинг в `DictionaryTabUseCaseImpl` — отдельный интеграционный сценарий, не часть mapper-тестов.)
