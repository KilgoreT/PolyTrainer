...
/**
 * Confirm-dialog удаления лексемы.
 */
@Composable
internal fun ConfirmDeleteLexemeWidget(
    onConfirm: () -> Unit,
    onDismiss: () -> Unit,
) {
    AlarmDialogWidget(
        ...
        Text(
            text = stringResource(id = R.string.word_card_delete_lexeme),
            style = LexemeStyle.H6,
            color = MaterialTheme.colorScheme.secondary,
        )
        Spacer(modifier = Modifier.height(8.dp))
        Text(
            text = stringResource(id = R.string.word_card_delete_lexeme_subheading),
